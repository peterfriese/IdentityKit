...
### Fixed
- **Toolbar close button**: Use SwiftUI canonical `Button(role: .close)` pattern instead of custom implementation
- Removed leaky `PlatformProxy+Toolbar` that was exposing implementation details
- Platform-specific placement: `.topBarTrailing` (iOS) / `.cancellationAction` (macOS)
